...
impl MiddleSearcher
{
    ...
    pub fn search(&self, board: &Board, middle_depth: usize, depth: usize) -> Result<(i32, Vec<Move>, u64), Interruption>
    {
        ...
        match leaf_idx {
            Some(leaf_idx) => Ok((value, neural_pvs[leaf_idx].clone(), node_count)),
            None => Ok((value, pvs[0].clone(), node_count)),
        }
    }
}
