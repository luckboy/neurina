...
impl NetworkV3
{
    ...
    { NetworkV3 { iw, ib, sw, sb, pw, pb, ow, ob, } }
...
impl Net for NetworkV3
{
            ...
            OF: FnMut(Matrix)  -> Result<(), Interruption>
    {
        ...
        let ow2 = self.ow.mul_elems(&self.ow);
        let ob2 = ob.mul_elems(&ob);
        for _ in 0..pv_count {
            z = &self.pw * &h + &pb;
            h = z.tanh();
            hf(h.clone())?;
    ...
    fn backpropagate(&self, i: &Matrix, hs: &[Matrix], os: &[Matrix], ys: &[Matrix], one: &Matrix) -> Self
    {
        ...
        let mut j = hs.len() - 1;
        let ow2 = self.ow.mul_elems(&self.ow);
        let ow_2 = &self.ow * -2.0;
        let ob_2 = &self.ob * -2.0;
